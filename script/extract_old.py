...
numericals = ['Sam.', 'Kin.', 'Chr.', 'Cor.', 'Thess.', 'Tim.', 'Pet.', 'John']  # yves NKJV
...
def simplify(data, outDir, book, file):  # HTML to JSON
    ...
    #     newLine = line
    #     for _hex, _char in LATIN_1_CHARS:
    #         newLine = newLine.replace(_hex, _char)
    #     outData.append(newLine)

    # with open(os.path.join(outDir, f'MONOLITH.html'), 'a') as f:
    #     try:
    ...
    for i in range(2, len(data)-1):
        ...
        # footnotes
        note = re.search(re.compile(r'<span class="note ."><span class="label">#</span><span class=" body">([^#]+?)</span></span>'), cleantext)
        while (note):
            noteContent = cleantext[note.regs[1][0]:note.regs[1][1]]

            # roman-numericalise
            for numerical in numericals:
                if numerical in noteContent:
                    for number in range(1,4):
                        noteContent = re.sub(re.compile(f'{number} {numerical}'), f'{"I" * number} {numerical}', noteContent)
            ...
            cleantext = re.sub(re.compile(r'<span class="note ."><span class="label">#</span><span class=" body">([^#]+?)</span></span>'), f'<span class="note">{noteContent}</span>', cleantext, 1)  # remove footnotes
            note = re.search(re.compile(r'<span class="note ."><span class="label">#</span><span class=" body">([^#]+?)</span></span>'), cleantext)
                    ...
                    if (child.name):  # child is an element
                        ...
                        if ('{' in child) or ('~' in child):  # meta
                            if ('{' in child):  # new paragraph
                                para = re.findall(re.compile(r'\{(p|pc|q1|q2|q3|s|s1|sp|ms|qa|m|pmr|q1|qs|qr)\}'), child)[0]
